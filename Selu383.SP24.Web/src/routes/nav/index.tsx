        ...
        <>
            <Navbar className="navbar navbar-light bg-light" style={{ position: 'sticky', top: 0, zIndex: 1000 }}>
                ...
                <ul className="navbar-nav ml-auto">
                    ...
                    {user === null && ( // Render the admin login button only if no user is logged in
                        <Link to={'/AdminLogin'}><Button style={{ backgroundColor: '#FDBA74' }} className="btn-light">Admin Login</Button></Link>
                    )}
                        {user !== null ? (
                            <Button style={{ backgroundColor: '#FDBA74' }} className="btn-light" onClick={handleSignout}>Logout</Button>
                        ) : (
